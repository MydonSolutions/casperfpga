...
class ADS5296fw:
    ...
    def init(self, cs):
        self.reset(cs)
        self._desperate_debug(
            "CS:%d 0x0 (wrote 1) read 0x%x" % (cs, self.read_spi(0, cs))
        )
        self.write_spi(0x0F, 0x0400, cs)  # power down pin partial
        self._desperate_debug(
            "CS:%d 0x0f (wrote 0x0400) read 0x%x" % (cs, self.read_spi(0x0F, cs))
        )
        self.write_spi(0x07, 0x0001, cs)  # enable interleave
        self._desperate_debug(
            "CS:%d 0x07 (wrote 0x0001) read 0x%x" % (cs, self.read_spi(0x07, cs))
        )
        # self.write_spi(0x42, 0x8000, cs) # DDR phase
        # self.logger._desperate_debug("CS:%d"%cs, hex(self.read_spi(0x42, cs)))
        self.write_spi(
            0x46, 0x8104, cs
        )  # 10b serialization, LSB first, 2's complement, DDR
        self._desperate_debug(
            "CS:%d 0x46 (wrote 0x8104) read 0x%x" % (cs, self.read_spi(0x46, cs))
        )
        self.write_spi(0x40, 0x8000, cs)  # input selection
        self._desperate_debug(
            "CS:%d 0x40 (wrote 0x8000) 0x%x" % (cs, self.read_spi(0x40, cs))
        )
        self.write_spi(0x25, 0x8000, cs)  # Enable external sync
        self._desperate_debug(
            "CS:%d 0x25 (wrote 0x8000) 0x%x" % (cs, self.read_spi(0x25, cs))
        )
    ...
    def _write_ctrl(self, data, offset=0, board=0):
        ...
        self.fpga.write_int(
            "ads5296_controller%d_%d" % (self.fmc, board),
            data,
            word_offset=offset,
            blindwrite=True,
        )
    ...
    def _read_ctrl(self, offset=0, board=0):
        return self.fpga.read_uint(
            "ads5296_controller%d_%d" % (self.fmc, board), word_offset=offset
        )
    ...
    def enable_test_pattern(self, mode, cs, val0=0x3FF, val1=0x000):
        if mode == "ramp":
            ...
            self.logger.error(
                "Error, test mode not understood. Try 'ramp', 'toggle', 'data', or 'constant'"
            )
            return
        val0 = val0 << 2
        val1 = val1 << 2
        ...
        self.write_spi(
            0x25, (1 << 15) + (val << 4) + ((val1 >> 10) << 2) + (val0 >> 10), cs
        )
        self.read_spi(
            0x26, cs
        )  # TODO. After reading address 0x25, future reads of some addresses return 0xff. ?!?!
    ...
    def _write(self, data, offset=0):
        ...
        self.fpga.write_int(
            self.spi_ctrl_reg, data, word_offset=offset, blindwrite=True
        )
    ...
    def write_spi(self, addr, data, chip, readback=True):
        self._desperate_debug(
            "FMC %d: Writing 0x%x to address 0x%x, CS:0x%x"
            % (self.fmc, data, addr, chip)
        )
        ...
        if readback and readback_data != data:
            self.logger.warning(
                "WARNING >>>> SPI readback error (FMC %d, chip %d, addr 0x%x)"
                % (self.fmc, chip, addr)
            )
