...
class SkarabAdc(object):
    ...
    def get_adc_embedded_software_version(self):
        ...
        self.parent.transport.write_i2c(
            ...
            sd.ADC_FIRMWARE_MAJOR_VERSION_REG,
        )
        major_version = self.parent.transport.read_i2c(
            self.i2c_interface, sd.STM_I2C_DEVICE_ADDRESS, 1
        )

        self.parent.transport.write_i2c(
            self.i2c_interface,
            sd.STM_I2C_DEVICE_ADDRESS,
            sd.ADC_FIRMWARE_MINOR_VERSION_REG,
        )
        minor_version = self.parent.transport.read_i2c(
            self.i2c_interface, sd.STM_I2C_DEVICE_ADDRESS, 1
        )
    ...
    def write_skarab_adc_register(self, write_address, write_byte):
        ...
        """
        self.parent.transport.write_i2c(
            self.i2c_interface, sd.STM_I2C_DEVICE_ADDRESS, write_address, write_byte
        )
    ...
    def read_skarab_adc_register(self, read_address):
        ...
        """
        self.parent.transport.write_i2c(
            self.i2c_interface, sd.STM_I2C_DEVICE_ADDRESS, read_address
        )
        read_byte = self.parent.transport.read_i2c(
            self.i2c_interface, sd.STM_I2C_DEVICE_ADDRESS, 1
        )[0]
        return read_byte
    ...
    def configure_skarab_adc(self, nyquist_zone, decimation_rate=4):
        ...
        if not (
            nyquist_zone == sd.FIRST_NYQ_ZONE or nyquist_zone == sd.SECOND_NYQ_ZONE
        ):
            print(
                "SkarabAdc.configure_skarab_adc ERROR: invalid value provided for nyquist_zone"
            )
            return
        # 1.3 CHECK DEVICE TAG
        if not (
            self.device_info["tag"] == "xps:skarab_adc4x3g_14"
            or self.device_info["tag"] == "xps:skarab_adc4x3g_14_byp"
        ):
        ...
        if not (decimation_rate in (4, 8, 16, 32)):
            ...
            return
        if (decimation_rate == 32) and not (
            self.device_info["dec_modes"] == "4,8,16,32"
        ):
            print(
                "SkarabAdc.configure_skarab_adc ERROR: Yellow Block is not configured to support decimate by 32"
            )
            return
        ...
        if device_tag == "xps:skarab_adc4x3g_14":
            if self.decimation_rate == 4:
                SkarabAdc.write_skarab_adc_register(
                    self, sd.BOARD_SAMPLE_MODE_REG, sd.DDCDEC4_3GSPS_SAMPLE_MODE
                )
            elif self.decimation_rate == 8:
                SkarabAdc.write_skarab_adc_register(
                    self, sd.BOARD_SAMPLE_MODE_REG, sd.DDCDEC8_3GSPS_SAMPLE_MODE
                )
            else:  # dec 16 or 32
                SkarabAdc.write_skarab_adc_register(
                    self, sd.BOARD_SAMPLE_MODE_REG, sd.DDCDEC16_3GSPS_SAMPLE_MODE
                )
        elif device_tag == "xps:skarab_adc4x3g_14_byp":
            SkarabAdc.write_skarab_adc_register(
                self, sd.BOARD_SAMPLE_MODE_REG, sd.FULLBW_2P8GSPS_SAMPLE_MODE
            )
        ...
        # --------------------------------------------------------------
        SkarabAdc.write_skarab_adc_register(
            self, sd.RECONFIG_BOARD_REG, sd.RECONFIG_BOARD
        )
        ...
        timeout = 0
        while (
            SkarabAdc.read_skarab_adc_register(self, sd.RECONFIG_BOARD_REG) != 0
        ) and (timeout < 100):
            time.sleep(0.1)
            timeout = timeout + 1
        if timeout == 100:
            print(
                "SkarabAdc.configure_skarab_adc ERROR: Wait for configuration completion timeout"
            )
            return

    def configure_skarab_adc_ddcs(
        self,
        channel,
        ddc0_centre_frequency=1e9,
        ddc1_centre_frequency=0,
        dual_band_mode_enable=False,
        real_ddc_output_enable=False,
        print_actual_frequency=False,
    ):
        """
        ...
        if isinstance(ddc0_centre_frequency, int) == False:
            print(
                "SkarabAdc.configure_skarab_adc_ddcs ERROR: ddc0_centre_frequency is not an int"
            )
            return
        if isinstance(ddc1_centre_frequency, int) == False:
            print(
                "SkarabAdc.configure_skarab_adc_ddcs ERROR: ddc1_centre_frequency is not an int"
            )
            return
        if isinstance(dual_band_mode_enable, bool) == False:
            print(
                "SkarabAdc.configure_skarab_adc_ddcs ERROR: dual_band_mode_enable is not a bool"
            )
            return
        if isinstance(real_ddc_output_enable, bool) == False:
            print(
                "SkarabAdc.configure_skarab_adc_ddcs ERROR: real_ddc_output_enable is not a bool"
            )
            return
        if isinstance(print_actual_frequency, bool) == False:
            print(
                "SkarabAdc.configure_skarab_adc_ddcs ERROR: print_actual_frequency is not a bool"
            )
            return
        ...
        if not (channel >= 0 and channel <= 3):
            ...
            return
        if not (
            ddc0_centre_frequency >= 0 and ddc0_centre_frequency <= adc_sample_rate
        ):
            print(
                "SkarabAdc.configure_skarab_adc_ddcs ERROR: Invalid DDC 0 frequency specified"
            )
            return
        if not (
            ddc1_centre_frequency >= 0 and ddc1_centre_frequency <= adc_sample_rate
        ):
            print(
                "SkarabAdc.configure_skarab_adc_ddcs ERROR: Invalid DDC 1 frequency specified"
            )
            return
        ...
        # --------------------------------------------------
        nco_0_register_setting_f = pow(2.0, 16.0) * (
            ddc0_centre_frequency / adc_sample_rate
        )
        ...
        # --------------------------------------------------
        nco_1_register_setting_f = pow(2.0, 16.0) * (
            ddc1_centre_frequency / adc_sample_rate
        )
        ...
        # --------------------------------------------------
        ddc0_centre_frequency_actual = (
            float(nco_0_register_setting) / pow(2.0, 16.0)
        ) * float(adc_sample_rate)
        ddc1_centre_frequency_actual = (
            float(nco_1_register_setting) / pow(2.0, 16.0)
        ) * float(adc_sample_rate)
        ...
        # --------------------------------------------------
        SkarabAdc.write_skarab_adc_register(
            self, sd.DDC0_NCO_SETTING_MSB_REG, nco_0_register_setting_msb
        )
        SkarabAdc.write_skarab_adc_register(
            self, sd.DDC0_NCO_SETTING_LSB_REG, nco_0_register_setting_lsb
        )
        ...
        if dual_band_mode_enable == True:
            SkarabAdc.write_skarab_adc_register(
                self, sd.DDC1_NCO_SETTING_MSB_REG, nco_1_register_setting_msb
            )
            SkarabAdc.write_skarab_adc_register(
                self, sd.DDC1_NCO_SETTING_LSB_REG, nco_1_register_setting_lsb
            )
        ...
        while (
            (
                SkarabAdc.read_skarab_adc_register(self, sd.DDC_CONTROL_REG)
                & sd.UPDATE_DDC_CHANGE
            )
            != 0
        ) and (timeout < 1000):
            time.sleep(0.1)
        ...
        if timeout == 1000:
            print(
                "SkarabAdc.configure_skarab_adc_ddcs ERROR: Timeout waiting for DDC configuration to complete"
            )
            return
    ...
    def set_skarab_adc_data_mode(self, data_mode):
        ...
        if not (
            data_mode == sd.ADC_DATA_MODE
            or data_mode == sd.RAMP_DATA_MODE
            or data_mode == sd.TP_LAYER_DATA_MODE
        ):
            print(
                "SkarabAdc.set_skarab_adc_data_mode ERROR: invalid value provided for data_mode"
            )
            return
        # 1.3 CHECK DEVICE TAG
        if not (
        ...
        if device_tag == "xps:skarab_adc4x3g_14":
            if data_mode == sd.RAMP_DATA_MODE:
                ...
                )  # Pattern Code for ChB: all_0=0x11, all_1=0x22, toggle(16h'AAAA/16h'5555)=0x33, Ramp=0x44, custom_single_pattern1=0x66, custom_double_pattern1&2=0x77
                self.parent.transport.direct_spi_write(
                ...
                )  # Pattern Code for ChA: all_0=0x11, all_1=0x22, toggle(16h'AAAA/16h'5555)=0x33, Ramp=0x44, custom_single_pattern1=0x66, custom_double_pattern1&2=0x77
                self.parent.transport.direct_spi_write(
                ...
                self.parent.transport.direct_spi_write(
                    self.mezzanine_site,
                    sd.SPI_DESTINATION_ADC_0,
                    sd.ADC_DIGITAL_BANK_PAGE_SEL_LSB,
                    0x00,
                )
                self.parent.transport.direct_spi_write(
                    self.mezzanine_site,
                    sd.SPI_DESTINATION_ADC_0,
                    sd.ADC_DIGITAL_BANK_PAGE_SEL_MID,
                    0x00,
                )
                self.parent.transport.direct_spi_write(
                    self.mezzanine_site,
                    sd.SPI_DESTINATION_ADC_0,
                    sd.ADC_DIGITAL_BANK_PAGE_SEL_MSB,
                    0x69,
                )
                self.parent.transport.direct_spi_write(
                    self.mezzanine_site,
                    sd.SPI_DESTINATION_ADC_0,
                    sd.ADC_JESD_CHAN_A_CTRL_K,
                    0x80,
                )
                self.parent.transport.direct_spi_write(
                    self.mezzanine_site,
                    sd.SPI_DESTINATION_ADC_0,
                    sd.ADC_JESD_CHAN_B_CTRL_K,
                    0x80,
                )
                self.parent.transport.direct_spi_write(
                    self.mezzanine_site,
                    sd.SPI_DESTINATION_ADC_0,
                    sd.ADC_JESD_CHAN_A_LINK_LAYER_TESTMODE,
                    0x00,
                )
                self.parent.transport.direct_spi_write(
                    self.mezzanine_site,
                    sd.SPI_DESTINATION_ADC_0,
                    sd.ADC_JESD_CHAN_B_LINK_LAYER_TESTMODE,
                    0x00,
                )
                self.parent.transport.direct_spi_write(
                    self.mezzanine_site,
                    sd.SPI_DESTINATION_ADC_1,
                    sd.ADC_DIGITAL_BANK_PAGE_SEL_LSB,
                    0x00,
                )
                self.parent.transport.direct_spi_write(
                ...
                self.parent.transport.direct_spi_write(
                    self.mezzanine_site, sd.SPI_DESTINATION_ADC_0, 0x5837, 0x00
                )  # Pattern Code for ChB: all_0=0x11, all_1=0x22, toggle(16h'AAAA/16h'5555)=0x33, Ramp=0x44, custom_single_pattern1=0x66, custom_double_pattern1&2=0x77
                self.parent.transport.direct_spi_write(
                    self.mezzanine_site, sd.SPI_DESTINATION_ADC_0, 0x5838, 0x00
                )  # Pattern Code for ChB: all_0=0x11, all_1=0x22, toggle(16h'AAAA/16h'5555)=0x33, Ramp=0x44, custom_single_pattern1=0x66, custom_double_pattern1&2=0x77
                self.parent.transport.direct_spi_write(
                    self.mezzanine_site, sd.SPI_DESTINATION_ADC_0, 0x583A, 0x00
                )
                ...
                self.parent.transport.direct_spi_write(
                    self.mezzanine_site, sd.SPI_DESTINATION_ADC_0, 0x5037, 0x00
                )  # Pattern Code for ChA: all_0=0x11, all_1=0x22, toggle(16h'AAAA/16h'5555)=0x33, Ramp=0x44, custom_single_pattern1=0x66, custom_double_pattern1&2=0x77
                self.parent.transport.direct_spi_write(
                    self.mezzanine_site, sd.SPI_DESTINATION_ADC_0, 0x5038, 0x00
                )  # Pattern Code for ChA: all_0=0x11, all_1=0x22, toggle(16h'AAAA/16h'5555)=0x33, Ramp=0x44, custom_single_pattern1=0x66, custom_double_pattern1&2=0x77
                self.parent.transport.direct_spi_write(
                    self.mezzanine_site, sd.SPI_DESTINATION_ADC_0, 0x503A, 0x00
                )
                ...
                self.parent.transport.direct_spi_write(
                    self.mezzanine_site, sd.SPI_DESTINATION_ADC_1, 0x5837, 0x00
                )  # Pattern Code for ChB: all_0=0x11, all_1=0x22, toggle(16h'AAAA/16h'5555)=0x33, Ramp=0x44, custom_single_pattern1=0x66, custom_double_pattern1&2=0x77
                self.parent.transport.direct_spi_write(
                    self.mezzanine_site, sd.SPI_DESTINATION_ADC_1, 0x5838, 0x00
                )  # Pattern Code for ChB: all_0=0x11, all_1=0x22, toggle(16h'AAAA/16h'5555)=0x33, Ramp=0x44, custom_single_pattern1=0x66, custom_double_pattern1&2=0x77
                self.parent.transport.direct_spi_write(
                    self.mezzanine_site, sd.SPI_DESTINATION_ADC_1, 0x583A, 0x00
                )
                ...
                self.parent.transport.direct_spi_write(
                    self.mezzanine_site, sd.SPI_DESTINATION_ADC_1, 0x5037, 0x00
                )  # Pattern Code for ChA: all_0=0x11, all_1=0x22, toggle(16h'AAAA/16h'5555)=0x33, Ramp=0x44, custom_single_pattern1=0x66, custom_double_pattern1&2=0x77
                self.parent.transport.direct_spi_write(
                    self.mezzanine_site, sd.SPI_DESTINATION_ADC_1, 0x5038, 0x00
                )  # Pattern Code for ChA: all_0=0x11, all_1=0x22, toggle(16h'AAAA/16h'5555)=0x33, Ramp=0x44, custom_single_pattern1=0x66, custom_double_pattern1&2=0x77
                self.parent.transport.direct_spi_write(
                    self.mezzanine_site, sd.SPI_DESTINATION_ADC_1, 0x503A, 0x00
                )
                ...
                print(
                    "SkarabAdc.set_skarab_adc_data_mode ERROR: Unknown SKARAB ADC board data mode"
                )
                return
        elif device_tag == "xps:skarab_adc4x3g_14_byp":
            if data_mode == sd.RAMP_DATA_MODE:
    ...
    def set_skarab_adc_channel_gain(self, channel, gain):
        ...
        if not (channel >= 0 and channel <= 3):
            print(
                "SkarabAdc.set_skarab_adc_channel_gain ERROR: invalid channel provided"
            )
            return
        ...
        # --------------------------------------------------------------
        SkarabAdc.write_skarab_adc_register(
            self, sd.GAIN_CONTROL_REG, gain_control_byte
        )
        ...
        while (
            (
                SkarabAdc.read_skarab_adc_register(self, sd.GAIN_CONTROL_REG)
                & sd.UPDATE_GAIN
            )
            != 0
        ) and (timeout < 100):
        ...
        if timeout == 100:
            print(
                "SkarabAdc.set_skarab_adc_channel_gain ERROR: Wait for gain update completion timeout"
            )
            return
    ...
    def enable_skarab_adc_dout(self, enable_or_disable):
        ...
        if isinstance(enable_or_disable, bool) == False:
            print(
                "SkarabAdc.enable_skarab_adc_dout ERROR: enable_or_disable is not a bool"
            )
            return
        ...
        if enable_or_disable == True:
            self.parent.transport.write_wishbone(
                wb_base_adr + sd.REGADR_WR_ENABLE_DOUT, 1
            )
        else:
            self.parent.transport.write_wishbone(
                wb_base_adr + sd.REGADR_WR_ENABLE_DOUT, 0
            )
    ...
    def reset_skarab_adc(self):
        ...
        # ---------------------------------------------------------------
        self.parent.transport.write_wishbone(
            wb_base_adr + sd.REGADR_WR_ADC_SYNC_START, 0
        )
        self.parent.transport.write_wishbone(
            wb_base_adr + sd.REGADR_WR_ADC_SYNC_PART2_START, 0
        )
        self.parent.transport.write_wishbone(
            wb_base_adr + sd.REGADR_WR_ADC_SYNC_PART3_START, 0
        )
        self.parent.transport.write_wishbone(
            wb_base_adr + sd.REGADR_WR_PLL_SYNC_START, 0
        )
        self.parent.transport.write_wishbone(
            wb_base_adr + sd.REGADR_WR_PLL_PULSE_GEN_START, 0
        )
        self.parent.transport.write_wishbone(
            wb_base_adr + sd.REGADR_WR_MEZZANINE_RESET, 0
        )
    ...
    def print_status(self):
        ...
        # ---------------------------------------------------------------
        adc0_status_out = (
            self.parent.transport.read_wishbone(wb_base_adr + sd.REGADR_RD_ADC0_STATUS)
        ) & 0xFFBFFFFF
        adc1_status_out = (
            self.parent.transport.read_wishbone(wb_base_adr + sd.REGADR_RD_ADC1_STATUS)
        ) & 0xFFBFFFFF
        adc2_status_out = (
            self.parent.transport.read_wishbone(wb_base_adr + sd.REGADR_RD_ADC2_STATUS)
        ) & 0xFFBFFFFF
        adc3_status_out = (
            self.parent.transport.read_wishbone(wb_base_adr + sd.REGADR_RD_ADC3_STATUS)
        ) & 0xFFBFFFFF
    ...
    def sync_skarab_adc(self, skarab_adc_slaves=[]):
        ...
        if len(skarab_adc_slaves) > 0:
            for i in range(len(skarab_adc_slaves)):
                if isinstance(skarab_adc_slaves[i], SkarabAdc) == False:
                    print(
                        "sync_skarab_adc ERROR: skarab_adc_slaves needs to contain SkarabAdc objects"
                    )
                    return
        ...
        for i in range(skarab_adc_num):
            skarab_adcs[i].parent.transport.write_i2c(
                skarab_adcs[i].i2c_interface,
                sd.STM_I2C_DEVICE_ADDRESS,
                sd.ADC_FIRMWARE_MAJOR_VERSION_REG,
            )
            major_version = skarab_adcs[i].parent.transport.read_i2c(
                skarab_adcs[i].i2c_interface, sd.STM_I2C_DEVICE_ADDRESS, 1
            )[0]
            skarab_adcs[i].parent.transport.write_i2c(
                skarab_adcs[i].i2c_interface,
                sd.STM_I2C_DEVICE_ADDRESS,
            ...
            minor_version = skarab_adcs[i].parent.transport.read_i2c(
                skarab_adcs[i].i2c_interface, sd.STM_I2C_DEVICE_ADDRESS, 1
            )[0]
            if not (major_version == 2 and minor_version >= 0):
                print(
                    str(
                        "sync_skarab_adc ERROR: SKARAB ADC "
                        + str(i)
                        + " not programmed with correct software version"
                    )
                )
                exit()
        ...
        for i in range(skarab_adc_num):
            skarab_adcs[i].parent.transport.write_wishbone(
                skarab_adcs[i].address + sd.REGADR_WR_ADC_SYNC_START, 0
            )
            skarab_adcs[i].parent.transport.write_wishbone(
                skarab_adcs[i].address + sd.REGADR_WR_ADC_SYNC_PART2_START, 0
            )
            skarab_adcs[i].parent.transport.write_wishbone(
                skarab_adcs[i].address + sd.REGADR_WR_ADC_SYNC_PART3_START, 0
            )
            skarab_adcs[i].parent.transport.write_wishbone(
                skarab_adcs[i].address + sd.REGADR_WR_PLL_SYNC_START, 0
            )
            skarab_adcs[i].parent.transport.write_wishbone(
                skarab_adcs[i].address + sd.REGADR_WR_PLL_PULSE_GEN_START, 0
            )
            skarab_adcs[i].parent.transport.write_wishbone(
                skarab_adcs[i].address + sd.REGADR_WR_MEZZANINE_RESET, 0
            )
        ...
        if device_tag == "xps:skarab_adc4x3g_14":
            for i in range(skarab_adc_num):
                if self.decimation_rate == 4:
                    skarab_adcs[i].parent.transport.write_wishbone(
                        skarab_adcs[i].address + sd.REGADR_WR_DECIMATION_RATE, 0
                    )
                elif self.decimation_rate == 8:
                    skarab_adcs[i].parent.transport.write_wishbone(
                        skarab_adcs[i].address + sd.REGADR_WR_DECIMATION_RATE, 1
                    )
                elif self.decimation_rate == 16:
                    skarab_adcs[i].parent.transport.write_wishbone(
                        skarab_adcs[i].address + sd.REGADR_WR_DECIMATION_RATE, 2
                    )
                else:  # dec 32
                    skarab_adcs[i].parent.transport.write_wishbone(
                        skarab_adcs[i].address + sd.REGADR_WR_DECIMATION_RATE, 3
                    )
        ...
        for i in range(skarab_adc_num):
            skarab_adcs[i].parent.transport.write_i2c(
                skarab_adcs[i].i2c_interface,
                sd.STM_I2C_DEVICE_ADDRESS,
                sd.HOST_PLL_GPIO_CONTROL_REG,
            )
            read_reg = skarab_adcs[i].parent.transport.read_i2c(
                skarab_adcs[i].i2c_interface, sd.STM_I2C_DEVICE_ADDRESS, 1
            )[0]
        ...
        for i in range(skarab_adc_num):
            ...
            skarab_adcs[i].parent.transport.direct_spi_write(
                skarab_adcs[i].mezzanine_site,
                sd.SPI_DESTINATION_PLL,
                sd.PLL_GLOBAL_MODE_AND_ENABLE_CONTROL,
                0x41,
            )
            ...
            skarab_adcs[i].parent.transport.direct_spi_write(
                skarab_adcs[i].mezzanine_site,
                sd.SPI_DESTINATION_PLL,
                sd.PLL_CHANNEL_OUTPUT_3_CONTROL_FORCE_MUTE,
                0x88,
            )
            skarab_adcs[i].parent.transport.direct_spi_write(
                skarab_adcs[i].mezzanine_site,
                sd.SPI_DESTINATION_PLL,
                sd.PLL_CHANNEL_OUTPUT_7_CONTROL_FORCE_MUTE,
                0x88,
            )
            skarab_adcs[i].parent.transport.direct_spi_write(
                skarab_adcs[i].mezzanine_site,
                sd.SPI_DESTINATION_PLL,
                sd.PLL_CHANNEL_OUTPUT_3_CONTROL_HIGH_PERFORMANCE_MODE,
                0xDD,
            )
            skarab_adcs[i].parent.transport.direct_spi_write(
                skarab_adcs[i].mezzanine_site,
                sd.SPI_DESTINATION_PLL,
                sd.PLL_CHANNEL_OUTPUT_7_CONTROL_HIGH_PERFORMANCE_MODE,
                0xDD,
            )
            # 6.3 ENABLE PLL SYNC
            skarab_adcs[i].parent.transport.write_i2c(
        ...
        skarab_adcs[0].parent.transport.write_wishbone(
            skarab_adcs[0].address + sd.REGADR_WR_PLL_SYNC_START, 0
        )
        skarab_adcs[0].parent.transport.write_wishbone(
            skarab_adcs[0].address + sd.REGADR_WR_PLL_SYNC_START, 1
        )
        # 7.2 WAIT FOR PLL SYNC COMPLETION
        timeout = 0
        read_reg = skarab_adcs[0].parent.transport.read_wishbone(
            skarab_adcs[0].address + sd.REGADR_RD_PLL_SYNC_COMPLETE
        )
        ...
        for i in range(skarab_adc_num):
            spi_read_word = skarab_adcs[i].parent.transport.direct_spi_read(
                skarab_adcs[i].mezzanine_site,
                sd.SPI_DESTINATION_PLL,
                sd.PLL_ALARM_READBACK,
            )
            timeout = 0
            while ((spi_read_word & sd.PLL_CLOCK_OUTPUT_PHASE_STATUS) == 0x0) and (
                timeout < 1000
            ):
                spi_read_word = skarab_adcs[i].parent.transport.direct_spi_read(
                    skarab_adcs[i].mezzanine_site,
        ...
        for i in range(skarab_adc_num):
            ...
            skarab_adcs[i].parent.transport.direct_spi_write(
                skarab_adcs[i].mezzanine_site,
                sd.SPI_DESTINATION_ADC_0,
                sd.ADC_GENERAL_ADC_PAGE_SEL,
                0x00,
            )
            skarab_adcs[i].parent.transport.direct_spi_write(
                skarab_adcs[i].mezzanine_site,
                sd.SPI_DESTINATION_ADC_0,
                sd.ADC_GENERAL_MASTER_PAGE_SEL,
                0x04,
            )
            skarab_adcs[i].parent.transport.direct_spi_write(
                skarab_adcs[i].mezzanine_site,
                sd.SPI_DESTINATION_ADC_0,
                sd.ADC_MASTER_PDN_SYSREF,
                0x00,
            )
            skarab_adcs[i].parent.transport.direct_spi_write(
                skarab_adcs[i].mezzanine_site,
        ...
        skarab_adcs[0].parent.transport.write_wishbone(
            skarab_adcs[0].address + sd.REGADR_WR_PLL_PULSE_GEN_START, 0
        )
        skarab_adcs[0].parent.transport.write_wishbone(
            skarab_adcs[0].address + sd.REGADR_WR_PLL_PULSE_GEN_START, 1
        )
        ...
        for i in range(skarab_adc_num):
            ...
            skarab_adcs[i].parent.transport.write_wishbone(
                skarab_adcs[i].address + sd.REGADR_WR_ADC_SYNC_START, 1
            )
            # 13.2 WAIT FOR ADC SYNC PART 1 COMPLETION
            timeout = 0
            read_reg = skarab_adcs[i].parent.transport.read_wishbone(
                skarab_adcs[i].address + sd.REGADR_RD_ADC_SYNC_COMPLETE
            )
            while (read_reg == 0) and (timeout < 100):
                read_reg = skarab_adcs[i].parent.transport.read_wishbone(
                    skarab_adcs[i].address + sd.REGADR_RD_ADC_SYNC_COMPLETE
        ...
        for i in range(skarab_adc_num):
            timeout = 0
            read_reg = skarab_adcs[i].parent.transport.read_wishbone(
                skarab_adcs[i].address + sd.REGADR_RD_ADC_SYNC_REQUEST
            )
            ...
            if timeout == 100:
                print(
                    str(
                        "sync_skarab_adc ERROR: SKARAB ADC "
                        + str(i)
                        + " wait for SYNC request assert timeout"
                    )
                )
                exit()
        ...
        skarab_adcs[0].parent.transport.write_wishbone(
            skarab_adcs[0].address + sd.REGADR_WR_ADC_SYNC_PART2_START, 0
        )
        skarab_adcs[0].parent.transport.write_wishbone(
            skarab_adcs[0].address + sd.REGADR_WR_ADC_SYNC_PART2_START, 1
        )
        # 15.2 WAIT FOR ADC SYNC PART 2 COMPLETION
        timeout = 0
        read_reg = skarab_adcs[0].parent.transport.read_wishbone(
            skarab_adcs[0].address + sd.REGADR_RD_ADC_SYNC_COMPLETE
        )
        while (read_reg == 0) and (timeout < 100):
            read_reg = skarab_adcs[0].parent.transport.read_wishbone(
                skarab_adcs[0].address + sd.REGADR_RD_ADC_SYNC_COMPLETE
        ...
        if timeout == 100:
            print(
                str(
                    "sync_skarab_adc ERROR: Master SKARAB ADC wait for ADC SYNC PART 2 timeout"
                )
            )
            exit()
            ...
            if timeout == 100:
                print(
                    str(
                        "sync_skarab_adc ERROR: SKARAB ADC "
                        + str(i)
                        + " wait for SYNC request de-assert timeout"
                    )
                )
                exit()
        ...
        skarab_adcs[0].parent.transport.write_wishbone(
            skarab_adcs[0].address + sd.REGADR_WR_ADC_SYNC_PART3_START, 0
        )
        skarab_adcs[0].parent.transport.write_wishbone(
            skarab_adcs[0].address + sd.REGADR_WR_ADC_SYNC_PART3_START, 1
        )
        ...
        if timeout == 100:
            print(
                str(
                    "sync_skarab_adc ERROR: Master SKARAB ADC wait for ADC SYNC PART 3 timeout"
                )
            )
            exit()
        ...
        for i in range(skarab_adc_num):
            adc0_status_out = (
                skarab_adcs[i].parent.transport.read_wishbone(
                    skarab_adcs[i].address + sd.REGADR_RD_ADC0_STATUS
                )
            ) & 0xFFBFFFFF
            adc1_status_out = (
                skarab_adcs[i].parent.transport.read_wishbone(
                    skarab_adcs[i].address + sd.REGADR_RD_ADC1_STATUS
                )
            ) & 0xFFBFFFFF
            adc2_status_out = (
                skarab_adcs[i].parent.transport.read_wishbone(
                    skarab_adcs[i].address + sd.REGADR_RD_ADC2_STATUS
                )
            ) & 0xFFBFFFFF
            adc3_status_out = (
                skarab_adcs[i].parent.transport.read_wishbone(
                    skarab_adcs[i].address + sd.REGADR_RD_ADC3_STATUS
                )
            ) & 0xFFBFFFFF
            if (
                adc0_status_out != 0xE0000000
                or adc1_status_out != 0xE0000000
                or adc2_status_out != 0xE0000000
                or adc3_status_out != 0xE0000000
            ):
                print(
                    str(
                        "sync_skarab_adc WARNING: SKARAB ADC "
                        + str(i)
        ...
        if (
            skarab_adcs[0].parent.transport.read_wishbone(
                skarab_adcs[0].address + sd.REGADR_RD_PLL_SYNC_COMPLETE
            )
            != 1
        ):
    ...
    def arm_and_trigger_and_capture(self, file_dir="."):
        ...
        for snapshot_object in self.parent.snapshots:
            ...
            adc_data_filename = "{}/{}_data.txt".format(
                abs_file_dir, snapshot_object.name
            )
