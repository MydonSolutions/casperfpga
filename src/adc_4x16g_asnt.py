...
class Adc_4X16G_ASNT(object):
    ...
    def __init__(self, parent, device_name, device_info, initalise=False):
        ...
        self.wbctrl = WishBoneDevice(
            self.parent, "adc4x16g_controller%d" % self.channel_sel
        )
    ...
    * StepRXSlide
    ...
    def setADC(self):
        if self.DAC_ON == 1:
            adc_a = (
                8 * self.ADC_params[0][3]
                + 4
                + 2 * self.ADC_params[0][1]
                + self.ADC_params[0][0]
            )
            adc_b = (
                8 * self.ADC_params[1][3]
                + 4
                + 2 * self.ADC_params[1][1]
                + self.ADC_params[1][0]
            )
            adc_c = (
                8 * self.ADC_params[2][3]
                + 4
                + 2 * self.ADC_params[2][1]
                + self.ADC_params[2][0]
            )
            adc_d = (
                8 * self.ADC_params[3][3]
                + 4
                + 2 * self.ADC_params[3][1]
                + self.ADC_params[3][0]
            )
        else:
            adc_a = (
    ...
    def check_alignment(self):
        ...
        if (
            ...
            & (match_pos[1] == match_pos[2])
            & (match_pos[2] == match_pos[3])
        ):
            return 0
        else:
            return 1
