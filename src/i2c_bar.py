...
class MS5611_01B(I2C_DEVICE):
    ...
    def readPress(self, rawtemp=None, dt=None, osr=4096):
        ...
        if rawtemp == None or dt == None:
            ...
            if rawtemp < 2000:
                t2 = (dt**2.0) / (1 << 31)
                off2 = 5 * ((rawtemp - 2000.0) ** 2) / 2
                sens2 = 5 * ((rawtemp - 2000.0) ** 2) / 4
            if rawtemp < -1500:
                off2 = off2 + 7 * ((rawtemp + 1500.0) ** 2)
                sens2 = sens2 + 11 * ((rawtemp + 1500.0) ** 2) / 2
        ...
        if (
            ...
            or off < self.OFF_MIN
            or off > self.OFF_MAX
            or sens < self.SENS_MIN
            or sens > self.SENS_MAX
        ):
            raise IOError("Compensation values excceed min-max range.")

        data = (data * 1.0 / (1 << 21) * sens - off) / (1 << 15)
