...
def fp2fixed(num, bitwidth, bin_pt, signed):
    ...
    if (not signed) and (num < 0):
        raise ValueError("Cannot represent negative number (%f) in %s" % (num, _format))
    if num == 0:
        return 0
    scaled = num * (2**bin_pt)
    scaled = round(scaled)
...
class Memory(bitfield.Bitfield):
    ...
    def __str__(self):
        return "%s%s: %ibits * %i, fields[%s]" % (
            self.name,
            "" if self.address == -1 else "@0x%08x" % self.address,
            self.width_bits,
            self.length_in_words(),
            self.fields_string_get(),
        )
    ...
    def _process_data(self, rawdata):
        ...
        if not isinstance(rawdata, bytes):
            raise TypeError(
                "self.read_raw returning incorrect datatype. " "Must be str or buffer."
            )
        ...
        for ctr, word in enumerate(memory_words):
            for field in self._fields.values():
                ...
                word_done = bin2fp(
                    word_shift, field.width_bits, field.binary_pt, field.numtype == 1
                )
