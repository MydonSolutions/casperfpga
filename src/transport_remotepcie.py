...
class RemotePcieTransport(Transport):
    ...
    def __init__(self, **kwargs):
        ...
        new_connection_msg = "*** NEW REST CLIENT MADE TO {} ***".format(
            self.server_uri
        )
    ...
    def blindwrite(self, device_name, data, offset=0):
        ...
        ):  # remote blindwrite returns the read result at the written location
            return response.content
        else:
            self.logger.warning(response.json())
            raise RuntimeError(response.json())
    ...
    def upload_to_ram_and_program(self, filename, wait_complete=None):
        ...
        response = self._put("/fpgfile", files={"fpga": open(filename, "rb")})
        if response.status_code == 200:
            self.logger.debug(response.json())
            return True
        else:
...
if __name__ == "__main__":
    ...
    if remotepcie.is_connected(0, 0) and not remotepcie.is_programmed():
        print(
            ...
            remotepcie.upload_to_ram_and_program(DEFAULT_FPGFILE),
        )
