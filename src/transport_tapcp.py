...
class TapcpTransport(Transport):
    ...
    def test_host_type(host_ip):
        ...
        try:
            ...
            self.logger.error(
                "tftpy is not installed, do not know if %s is a Tapcp"
    ...
    def test_host_type(host_ip):
        ...
        try:
            ...
            buf = BytesIO()
            board.download("%s.%x.%x" % ("sys_clkcounter", 0, 1), buf, timeout=3)
            try:
                board.context.end()
            except:
                pass
    ...
    def get_metadata(self):
        ...
        while meta.find("?end".encode()) == -1:
            meta_page = self.read(
                "/flash", READ_CHUNK_SIZE, offset=user_flash_loc + page_offset
            )
    ...
    def _update_metadata(self, filename, hlen, plen, md5):
        ...
        metadict["prog"] = "?prog_bitstream_start\t%d?prog_bitstream_length\t%d" % (
            prog_loc,
            plen,
        )
    ...
    def write_to_flash(self, payload, address):
        ...
        for i in pb(range(complete_blocks)):
            self.logger.debug(
                "block %d of %d: writing %d bytes to address 0x%x:"
                % (
                    i + 1,
                    complete_blocks,
                    len(payload[i * FLASH_SECTOR_SIZE : (i + 1) * FLASH_SECTOR_SIZE]),
                    address + i * FLASH_SECTOR_SIZE,
                )
            )
            self.blindwrite(
                "/flash",
                payload[i * FLASH_SECTOR_SIZE : (i + 1) * FLASH_SECTOR_SIZE],
                offset=address + i * FLASH_SECTOR_SIZE,
            )
            readback = self.read(
                "/flash",
                len(payload[i * FLASH_SECTOR_SIZE : (i + 1) * FLASH_SECTOR_SIZE]),
        ...
        if trailing_bytes:
            ...
            self.blindwrite(
                "/flash", payload[last_offset:], offset=address + last_offset
            )
            readback = self.read(
                "/flash", len(payload[last_offset:]), offset=address + last_offset
            )
    ...
    def upload_to_ram_and_program(
        self, filename, port=None, timeout=None, wait_complete=True, force=False
    ):
        user_flash_loc = self.get_user_flash_loc()
        ...
        old_timeout = self.timeout
        self.logger.debug(
            "Old timeout was %f. Setting new timeout to 1.5s" % old_timeout
        )
        self.timeout = 1.5
        if filename.endswith(".fpg"):
            self.logger.info(
                "Programming with an .fpg file. Checking if it is already in flash"
            )
            header, prog, md5 = self._extract_bitstream(filename)
            # Align header with sector size
            if len(header) % FLASH_SECTOR_SIZE:
                header += b"\00" * (
                    FLASH_SECTOR_SIZE - (len(header) % FLASH_SECTOR_SIZE)
                )
            self.logger.debug("Reading meta-data from flash")
            meta_inflash = self.get_metadata()
            if (
                (meta_inflash is not None)
                and (meta_inflash.get("md5sum", None) == md5)
                and (not force)
            ):
                ...
                head_loc, prog_loc = self._update_metadata(
                    filename, len(header), len(prog), md5
                )
                payload = header + prog
                complete_blocks = len(payload) // FLASH_SECTOR_SIZE
                trailing_bytes = len(payload) % FLASH_SECTOR_SIZE
                ...
                for i in pb(range(complete_blocks)):
                    self.logger.debug(
                        ...
                        % (
                            ...
                            len(
                                payload[
                                    i * FLASH_SECTOR_SIZE : (i + 1) * FLASH_SECTOR_SIZE
                                ]
                            ),
                            head_loc + i * FLASH_SECTOR_SIZE,
                        )
                    )
                    self.blindwrite(
                        "/flash",
                        payload[i * FLASH_SECTOR_SIZE : (i + 1) * FLASH_SECTOR_SIZE],
                        offset=head_loc + i * FLASH_SECTOR_SIZE,
                    )
                    readback = self.read(
                        "/flash",
                        len(
                    ...
                    if (
                        ...
                        print(
                            payload[i * FLASH_SECTOR_SIZE : i * FLASH_SECTOR_SIZE + 10]
                        )
                        print(
                            payload[
                                (i + 1) * FLASH_SECTOR_SIZE
                                - 10 : (i + 1) * FLASH_SECTOR_SIZE
                            ]
                        )
                        print(readback[-10:])
                ...
                if trailing_bytes:
                    ...
                    last_offset = complete_blocks * FLASH_SECTOR_SIZE
                    self.blindwrite(
                        "/flash", payload[last_offset:], offset=head_loc + last_offset
                    )
                    readback = self.read(
                        "/flash",
                        len(payload[last_offset:]),
                        offset=head_loc + last_offset,
                    )
                    if payload[last_offset:] != readback:
                        raise RuntimeError("Readback of flash failed!")
            ...
            for i in pb(range(complete_blocks)):
                ...
                self.blindwrite(
                    "/flash",
                    payload[i * FLASH_SECTOR_SIZE : (i + 1) * FLASH_SECTOR_SIZE],
                    offset=user_flash_loc + i * FLASH_SECTOR_SIZE,
                )
                readback = self.read(
                    "/flash",
                    len(payload[i * FLASH_SECTOR_SIZE : (i + 1) * FLASH_SECTOR_SIZE]),
            ...
            if trailing_bytes:
                ...
                last_offset = complete_blocks * FLASH_SECTOR_SIZE
                self.blindwrite(
                    "/flash", payload[last_offset:], offset=user_flash_loc + last_offset
                )
                readback = self.read(
                    "/flash",
                    len(payload[last_offset:]),
                    offset=user_flash_loc + last_offset,
                )
                if payload[last_offset:] != readback:
                    raise RuntimeError("Readback of flash failed!")
            self.logger.debug("Returning timeout to %f" % old_timeout)
    ...
    def _program_new_golden_image(self, imagefile):
        ...
        **Beware:** If this command fails, and you reboot your
        ...
        for i in pb(range(complete_blocks)):
            self.blindwrite(
                "/flash",
                payload[i * sector_size : (i + 1) * sector_size],
    ...
    def read(self, device_name, size, offset=0, use_bulk=True):
        ...
        for retry in range(self.retries - 1):
            try:
                buf = BytesIO()
                self.t.download(
                    "%s.%x.%x" % (device_name, offset, size), buf, timeout=self.timeout
                )
                try:
                    self.t.context.end()
                except:
        ...
        try:
            ...
            try:
                ...
                pass
        raise RuntimeError(
            "Failed to read size %d from register %s at offset %d"
            % (size, device_name, offset)
        )
    ...
    def blindwrite(self, device_name, data, offset=0, use_bulk=True):
        ...
        ), "Must supply binary packed string data"
        ...
        for retry in range(self.retries - 1):
            try:
                buf = BytesIO(data)
                self.t.upload(
                    "%s.%x.0" % (device_name, offset), buf, timeout=self.timeout
                )
                try:
                    self.t.context.end()
                except:
        ...
        try:
            ...
            try:
                ...
                pass
        raise RuntimeError(
            "Failed to write to register %s at offset %d" % (device_name, offset)
        )
