...
class Register(Memory):
    ...
    def from_device_info(
        ...
        if address == -1 or length_bytes == -1:
            ...
            raise RuntimeError(
                "Could not find address or length for " "Register %s" % device_name
            )
    ...
    def _write_common(self, **kwargs):
        ...
        for k in kwargs:
            if k not in new_values:
                raise ValueError(
                    "Field {} not found in register {} on host "
        ...
        for k in kwargs:
            if kwargs[k] == "pulse":
                ...
                new_values[k] = kwargs[k]
                LOGGER.debug(
                    "%s: writing %.5f to field %s" % (self.name, new_values[k], k)
                )
        ...
        if fixed_int > (2**32) - 1:
            LOGGER.error(
                "%s: problem writing to register %s:" % (self.parent.host, self.name)
            )
            for _f in self._fields.values():
                _intval = fp2fixed_int(
                    new_values[_f.name], _f.width_bits, _f.binary_pt, _f.numtype == 1
    ...
    def write_single(self, value):
        ...
        if len(self.field_names()) != 1:
            raise ValueError(
                "Register has more than one field, cannot "
    ...
    def _process_info_current(self):
        ...
        fields = {
            "names": clean_fields(self.name, "register", self.block_info["names"]),
            "widths": clean_fields(self.name, "register", self.block_info["bitwidths"]),
            "types": clean_fields(
                self.name, "register", self.block_info["arith_types"]
            ),
            "bps": clean_fields(self.name, "register", self.block_info["bin_pts"]),
        }
