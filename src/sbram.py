...
class Sbram(Memory):
    ...
    def __init__(
        self, parent, name, address, length_bytes, width_bits, device_info=None
    ):
        super(Sbram, self).__init__(
    ...
    def from_device_info(
        ...
        if address == -1 or length_bytes == -1:
            raise RuntimeError(
                "Could not find address or length for " "Sbram %s" % device_name
            )
        return cls(parent, device_name, address, length_bytes, width_bits, device_info)
