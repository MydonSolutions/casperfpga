...
class TenGbe(Memory, Gbe):
    ...
    def _check_memmap_compliance(self):
        ...
        if (
            (cpu_tx_en > 1)
            or (cpu_rx_en > 1)
            or ((core_type != 4) and (core_type != 2))
        ):
            return False
        else:
            return True
    ...
    def _memmap_write(self, register, value):
        ...
        if bytesize in (1, 2):
            ...
            raise RuntimeError(
                "Can only write 1,2,4,8 Byte registers with this function."
            )
    ...
    def _memmap_read(self, register):
        ...
        if bytesize in (4, 8):
            ...
            if bytesize == 2 and offset % 4 not in (0, 2):
                raise RuntimeError(
                    "Attempted to read 16-bits from 32-bit word with %iB offset. "
            ...
            raise RuntimeError(
                "Cannot read %s of size %i: only 1,2,4,8 B supported"
                % (register, bytesize)
            )
        return value
    ...
    def _memmap_write_array(self, register, value, ctype="L"):
        ...
        if len(value) != n_elem:
            raise RuntimeError(
                "Register is %i 32-bit words long, but array is "
    ...
    def configure_core(self, mac, ip, port, gateway=None, subnet_mask=None):
        ...
        subnet_mask_to_write = (
            IpAddress(subnet_mask).ip_int if subnet_mask is not None else 0xFFFFFF00
        )
    ...
    def get_gbe_core_details(
        self, read_arp=False, read_cpu=False, read_multicast=False
    ):
        """
