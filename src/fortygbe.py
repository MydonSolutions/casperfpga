...
class FortyGbe(TenGbe):
    ...
    def __init__(
        self, parent, name, address, length_bytes, device_info=None, position=None
    ):
        """
        ...
        super(FortyGbe, self).__init__(parent, name, address, length_bytes, device_info)
    ...
    def _check_memmap_compliance(self):
        ...
        self.logger.debug(
            "%s: cpu_tx_en %d, cpu_rx_en: %d, rev: %d, core_type: %d"
            % (self.name, cpu_tx_en, cpu_rx_en, rev, core_type)
        )
        if (
            (cpu_tx_en > 1)
            or (cpu_rx_en > 1)
            or ((core_type != 4) and (core_type != 3))
        ):
            return False
        else:
            return True
            ...
            if register.name.find(self.name + "_") == 0:
                ...
                if name[0:2] == "tx" and name.find("txs_") == -1:
                    ...
                    if not (name.find("txs_") == 0 or name.find("rxs_") == 0):
                        self.parent.logger.warn(
                            "%s: odd register name %s under Gbe "
    ...
    def get_stats(self):
        ...
        if int(self.block_info["debug_ctr_width"]) != 32:
            raise RuntimeError(
                "Please recompile your design with larger,"
        ...
        if txvldcnt in first:
            if second[txvldcnt] >= first[txvldcnt]:
                ...
                rv["tx_gbps"] = (
                    2 * 256 / 1e9 * (second[txvldcnt] - first[txvldcnt] + (2**32))
                )
        ...
        if rxvldcnt in first:
            if second[rxvldcnt] >= first[rxvldcnt]:
                ...
                rv["rx_gbps"] = (
                    2 * 256 / 1e9 * (second[rxvldcnt] - first[rxvldcnt] + (2**32))
                )
