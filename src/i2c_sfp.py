...
class Sfp:
    ...
    def __init__(self, itf, target_clock_khz=100.0, ref_clk_mhz=100.0):
        self.itf = itf
    ...
    def get_status(self):
        ...
        if diag_type["tx_pow"]:
            ...
            for i in range(4):
                tx_pow += [
                    self._conv_power(
                        x[self.OFFSET_TX_POWER + 2 * i],
                        x[self.OFFSET_TX_POWER + 2 * i + 1],
                    )
                ]
        else:
        ...
        for i in range(4):
            rx_pow += [
                self._conv_power(
                    x[self.OFFSET_RX_POWER + 2 * i], x[self.OFFSET_RX_POWER + 2 * i + 1]
                )
            ]
        ...
        vendor_pn_list = x[
            self.OFFSET_VENDOR_PN : self.OFFSET_VENDOR_PN + self.NBYTES_VENDOR_PN
        ]
        vendor_pn_str = "".join([chr(c) for c in vendor_pn_list])
        vendor_sn_list = x[
            self.OFFSET_VENDOR_SN : self.OFFSET_VENDOR_SN + self.NBYTES_VENDOR_SN
        ]
        vendor_sn_str = "".join([chr(c) for c in vendor_sn_list])
        vendor_rev_list = x[
            self.OFFSET_VENDOR_REV : self.OFFSET_VENDOR_REV + self.NBYTES_VENDOR_REV
        ]
        vendor_rev_str = "".join([chr(c) for c in vendor_rev_list])
        wavelength = self._conv_wavelength(
            x[self.OFFSET_WAVELENGTH], x[self.OFFSET_WAVELENGTH + 1]
        )
    ...
    def _conv_volt(self, msb, lsb):
        ...
        """
        volt = (msb << 8) + lsb
        volt *= 100.0e-6  # measurement in units of 100 uV
        return volt
