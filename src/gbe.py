...
class Gbe(object):
    ...
    def from_device_info(
        ...
        if address == -1 or length_bytes == -1:
            raise RuntimeError(
                "Could not find address or length " "for Gbe device %s" % device_name
            )
    ...
    def process_device_info(self, device_info):
        ...
        if mac is None or ip_address is None or port is None:
            raise ValueError(
                "%s: 10Gbe interface must " "have mac, ip and port." % self.fullname
            )
    ...
    def setup(self, mac, ipaddress, port, gateway=None, subnet_mask=None):
        ...
        raise NotImplementedError(
            "This is no longer required as the mac, "
    ...
    def print_gbe_core_details(self, arp=False, cpu=False, refresh=True):
        ...
        print(
            "Fabric interface is currently: %s" % "Enabled"
            if details["fabric_en"]
            else "Disabled"
        )
            ...
            if printmac:
                print(
                    ...
                    % (
                        details["ip_prefix"],
                        ip_address,
                        details["arp"][ip_address].mac_str,
                    )
                )
    ...
    def print_cpu_details(self, refresh=False):
        ...
        print(
            "CPU packet RX buffer unacknowledged data: %i"
