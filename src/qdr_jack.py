...
class QdrJack(Memory):
    ...
    def from_device_info(cls, parent, device_name, device_info, memorymap_dict):
        ...
        if mem_address == -1 or mem_length == -1:
            raise RuntimeError(
                "Could not find address or length for " "Qdr %s" % device_name
            )
        ...
        if ctrlreg_address == -1 or ctrlreg_length == -1:
            raise RuntimeError(
                "Could not find ctrl reg  address or length " "for Qdr %s" % device_name
            )
    ...
    def qdr_delay_out_step(self, bitmask, step):
        ...
        if step > 0:
            self.parent.write_int(
                self.control_mem, 0xFFFFFFFF, blindwrite=True, word_offset=7
            )
        elif step < 0:
        ...
        for i in range(abs(step)):
            ...
            self.parent.write_int(
                self.control_mem, 0xFFFFFFFF & bitmask, blindwrite=True, word_offset=6
            )
            self.parent.write_int(
                self.control_mem,
                (0xF & (bitmask >> 32)) << 4,
    ...
    def qdr_delay_clk_step(self, step):
        ...
        for i in range(abs(step)):
            self.parent.write_int(self.control_mem, 0, blindwrite=True, word_offset=5)
            self.parent.write_int(
                self.control_mem, 1 << 8, blindwrite=True, word_offset=5
            )
    ...
    def qdr_delay_in_step(self, bitmask, step):
        ...
        for i in range(abs(step)):
            ...
            self.parent.write_int(self.control_mem, 0, blindwrite=True, word_offset=5)
            self.parent.write_int(
                self.control_mem, (0xFFFFFFFF & bitmask), blindwrite=True, word_offset=4
            )
            self.parent.write_int(
                self.control_mem,
                ((0xF) & (bitmask >> 32)),
    ...
    def qdr_cal_check(self, verbosity=0):
        ...
        for pattern in CAL_DATA:
            ...
            self.parent.blindwrite(
                self.memory, struct.pack(">%iL" % len(pattern), *pattern)
            )
            retdat = struct.unpack(
                ">%iL" % len(pattern), self.parent.read(self.memory, len(pattern) * 4)
            )
    ...
    def find_in_delays(self, verbosity=0):
        ...
        for step in range(n_steps):
            ...
            for pattern in CAL_DATA:
                ...
                retdat = struct.unpack(
                    ">%iL" % len(pattern),
                    self.parent.read(self.memory, len(pattern) * 4),
                )
        ...
        for bit in range(n_bits):
            ...
            if cal_area[0] < 4:
                raise RuntimeError(
                    "Could not find a robust calibration setting for QDR %s" % self.name
                )
            # original was 1/2
    ...
    def qdr_check_cal_any_good(self, verbosity=0):
        ...
        for pn, pattern in enumerate(CAL_DATA):
            self.parent.blindwrite(
                self.memory,
                struct.pack(">%iL" % len(pattern), *pattern),
                offset=2**22,
            )
            retdat = struct.unpack(
                ">%iL" % len(pattern),
                self.parent.read(self.memory, len(pattern) * 4, offset=2**22),
            )
            for word_n, word in enumerate(pattern):
                patfail = patfail | (word ^ retdat[word_n])
                if verbosity > 2:
    ...
    def qdr_cal(self, fail_hard=True, verbosity=0):
        ...
        while (not cal) and (out_step < 32):
            ...
            if verbosity > 0:
                print(
                    "--- === Trying with OUT DELAYS to %i === ---" % out_step, end=" "
                )
