...
class SnapAdc(object):
    ...
    def getRegister(self, rid=None):
        if rid == None:
            ...
            return {
                name: self._get(rval, mask) for name, mask in self.WB_DICT[rid].items()
            }
        else:
            raise ValueError("Invalid Parameter")
    ...
    def bitslip(self, chipSel=None, laneSel=None):
        ...
        if not isinstance(chipSel, list):
            raise ValueError("Invalid parameter")
        elif isinstance(chipSel, list) and any(
            cs not in self.adcList for cs in chipSel
        ):
            raise ValueError("Invalid parameter")
        ...
        if not isinstance(laneSel, list):
            raise ValueError("Invalid parameter")
        elif isinstance(laneSel, list) and any(
            cs not in self.laneList for cs in laneSel
        ):
            raise ValueError("Invalid parameter")
        ...
        for cs in chipSel:
            for ls in laneSel:
                ...
                if verify:
                    assert (
                        self.adc._read(self.A_WB_W_CTRL) & self.M_WB_W_DELAY_TAP == 0x00
                    )
                self.adc._write(val, self.A_WB_W_CTRL)
                if verify:
                    rv = self.adc._read(self.A_WB_W_CTRL)
                    assert (
                        rv & self.M_WB_W_ISERDES_BITSLIP_CHIP_SEL
                        == val & self.M_WB_W_ISERDES_BITSLIP_CHIP_SEL
                    )
                    assert (
                        rv & self.M_WB_W_ISERDES_BITSLIP_LANE_SEL
                        == val & self.M_WB_W_ISERDES_BITSLIP_LANE_SEL
                    )
                self.adc._write(0x0, self.A_WB_W_CTRL)
                if verify:
                    assert (
    ...
    def delay(self, tap, chipSel=None, laneSel=None, verify=False):
        ...
        logger.debug(
            "Set DelayTap of lane {0} of chip {1} to {2}".format(
                str(laneSel), str(chipSel), tap
            )
        )
        ...
        if matla.size:
            mata = np.repeat(matc.reshape(-1, 1), matla.size, 1) + np.repeat(
                matla.reshape(1, -1), matc.size, 0
            )
        ...
        if matlb.size:
            matb = np.repeat(matc.reshape(-1, 1), matlb.size, 1) + np.repeat(
                matlb.reshape(1, -1), matc.size, 0
            )
    ...
    def test_patterns(
        self, chipSel=None, taps=None, mode="std", pattern1=None, pattern2=None
    ):
        ...
        def _check(data):
            ...
            if mode == "std" and pattern2 == None:  # std mode, single pattern
                ...
                counts = [
                    np.unique(d[:, i], return_counts=True)[1] for i in range(d.shape[1])
                ]
                ...
                diff = d[1:, :] - d[:-1, :]
                counts = [
                    np.unique(diff[:, ls], return_counts=True)[1]
                    for ls in self.laneList
                ]
        ...
        if taps == None:
            ...
            results = (
                np.array(results).reshape(len(chipSel), len(self.laneList)).tolist()
            )
            ...
            try:
                ...
                for t in range(ntaps):
                    ...
                    for chip, d in self.readRAM(signed=False).items():
                        h[chip, t] = (
                            np.sum(d == d[:1], axis=(0, 1)) == d.shape[0] * d.shape[1]
                        )
                self.selectADC()  # select all chips
                self.adc.test("off")
    ...
    def alignLineClock(self, chips_lanes=None, ker_size=5):
        ...
        self.logger.info("Aligning line clock on ADCs/lanes: %s" % str(chips_lanes))
        try:
    ...
    def alignFrameClock(self, chips_lanes=None, retry=True):
        ...
        if chips_lanes is None:
            chips_lanes = {chip: self.laneList for chip in self.adcList}
        self.logger.debug("Aligning frame clock on ADCs/lanes: %s" % str(chips_lanes))
        failed_chips = {}
        self.setDemux(numChannel=1)
        for chip, lanes in chips_lanes.items():
            self.selectADC(chip)
            ...
            for cnt in range(2 * self.resolution):
                ...
                failed_lanes += [
                    L
                    for L in lanes
                    if np.any(d[0::2, L] != d[0, L]) or np.any(d[1::2, L] != d[1, L])
                ]
        ...
        if len(failed_chips) > 0 and retry:
            if self._retry_cnt < self._retry:
                self._retry_cnt += 1
                self.logger.info(
                    "retry=%d/%d redo Line on ADCs/lanes: %s"
    ...
    def rampTest(self, nchecks=300, retry=False):
        ...
        if len(failed_chips) > 0 and retry:
            if self._retry_cnt < self._retry:
                self._retry_cnt += 1
                self.logger.info(
                    "retry=%d/%d redo Line/Frame on ADCs/lanes: %s"
                    % (self._retry_cnt, self._retry, failed_chips)
                )
                self.alignLineClock(failed_chips)
