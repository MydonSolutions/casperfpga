...
class TransportTarget(object):
    def __init__(self, xdma_id, cfpga=None, **kwargs):
        ...
        if self.cfpga is None:
            ...
            self.cfpga = casperfpga.CasperFpga(
                host=self.target, transport=casperfpga.LocalPcieTransport, **kwargs
            )
    ...
    def upload_to_ram_and_program(self, fpgfile_path):
        if fpgfile_path != self.fpgfile_path:
            if (
                self.fpgfile_path is not None
                and os.path.exists(self.fpgfile_path)
                and self.fpgfile_path.startswith(app.config["UPLOAD_FOLDER"])
            ):
                LOGGER.info(
                    'Removing previously uploaded fpg file for "{}": "{}"'.format(
                        self.target, fpgfile_path
                    )
                )
...
class RestTransport_Device(Resource):
    def get(self, target, device_name):
        ...
        try:
            ...
            response = make_response(
                {
                    "response": "Failed to read. Confirm device_name '{}'.".format(
                        device_name
                    ),
                },
                404,
    ...
    def put(self, target, device_name):
        ...
        try:
            ...
            bytes_read = transportTarget.cfpga.transport.read(
                device_name, len(data), offset
            )
...
class RestTransport_FpgFile(Resource):
    def get(self, target):
        ...
        try:
            ...
            response = {
                "response": "Failed to send fpgafile: {}".format(
                    transportTarget.fpgfile_path
                )
            }
            code = 500
            return response, code
    ...
    def put(self, target):
        ...
        try:
            ...
            if not transportTarget.upload_to_ram_and_program(filepath):
                response = {
                    "response": (
                        "Provided fpg file does not match "
                        '`pr_template`: "{}"'.format(transportTarget.fpg_template)
                    )
                }
                code = 500
            else:
                response = {"response": transportTarget.fpgfile_path}
                code = 200
        except:
            if filepath is not None:
                response = {"response": 'Error programming with "{}".'.format(filepath)}
                code = 500
            else:
                response = {
...
class RestTransport_IsConnected(Resource):
    def get(self, target):
        ...
        try:
            response = {"response": transportTarget.is_connected(timeout, retries)}
            code = 200
        except:
            response = {"response": "TransportTarget not created"}
            code = 500
        return response, code
...
class RestTransport_IsProgrammed(Resource):
    def get(self, target):
        ...
        response = {"response": transportTarget.fpgfile_path is not None}
        code = 200
        return response, code
...
class RestTransport_PciXdmaMap(Resource):
    def get(self):
        return {"pci_xdma_id_map": PCIE_XDMA_DICT}, 200


class RestTransport_Version(Resource):
    def get(self):
        return {"response": "1.2.0"}, 200
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "Initialize a REST server exposing localpcietransports as" "remote ones."
        )
    )
    parser.add_argument(
        "fpgfile",
    ...
    for (pci_id, xdma_id) in list(PCIE_XDMA_DICT.items()):
        ...
        if args.program:
            print(
                (
                    'Programmed "pcie{}" successfully: {}'.format(
                        pci_id, localtransport.upload_to_ram_and_program(args.fpgfile)
                    )
                )
            )
        else:
