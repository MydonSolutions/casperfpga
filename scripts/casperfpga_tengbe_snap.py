...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Check SPEAD packet format against " "supplied values.",
)
parser.add_argument(
    "--spead_version",
    dest="spead_version",
...
parser.add_argument(
    "--hex", dest="hex", action="store_true", default=False, help="show numbers in hex"
)
parser.add_argument(
    "--comms",
    dest="comms",
...
if args.spead:
    ...
    for ctr, spead_pkt in enumerate(spead_processor.packets):
        spead_pkt_strings = spead_pkt.get_strings(headers_only=False, hex_nums=args.hex)
